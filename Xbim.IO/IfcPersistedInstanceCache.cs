...
{   
    public class IfcPersistedInstanceCache : IDisposable
    {
        ...
        /// </summary>
        static readonly HashSet<IfcPersistedInstanceCache> OpenInstances;
        ...
        public IfcPersistedInstanceCache(XbimModel model, IEntityFactory factory)
        {
            _factory = factory;
                    ...
                    using (var zipStream = new ZipInputStream(fileStream))
                    {
                        ...
                        while (entry != null)
                        {
                            ...
                            if (extension != null)
                            {
                                ...
                                else if(string.CompareOrdinal(ext, ".ifcxml") == 0)
                                {
                                    using (var zipFile = new ZipFile(toImportFilename))
                                    {
                                        using (var transaction = table.BeginLazyTransaction())
        ...
        private Instance CreateInstance(string instanceName, string tempDirectory = null,  bool recovery = false)
        {
            ...
            _systemPath = !string.IsNullOrWhiteSpace(tempDirectory) ? tempDirectory : GetXbimTempDirectory();
       ...
       /// <returns></returns>
        ...
        public IEnumerable<TIfcType> OfType<TIfcType>(bool activate = false, int? indexKey = null, IfcType overrideType = null) where TIfcType:IPersistEntity 
        {
            ...
            var searchingIfcType = overrideType ?? IfcMetaData.IfcType(typeof(TIfcType));
        ...
        internal T InsertCopy<T>(T toCopy, XbimInstanceHandleMap mappings, XbimReadWriteTransaction txn, bool includeInverses) where T : IPersistEntity
        {
            ...
            txn.Pulse();
            var ifcType = IfcMetaData.IfcType(toCopy as IInstantiableEntity);
