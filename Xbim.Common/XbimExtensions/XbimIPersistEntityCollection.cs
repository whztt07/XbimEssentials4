...
namespace Xbim.Common.XbimExtensions
{
    public class XbimIPersistEntityCollection<TType> : ICollection<TType> where TType : class, IPersistEntity 
    {
        ...
        public int Count
        {
            get
            {
                return _dictionary.Values.Sum(dir => dir.Count);
            }
        }
        ...
        public bool Remove(TType item)
        {
            HashSet<TType> found;
            var fnd = _dictionary.TryGetValue(item.Model, out found);
            return fnd && found.Remove(item);
        }

        public IEnumerator<TType> GetEnumerator()
        {
            return new XbimIPersistEntityCollectionEnumerator(_dictionary);
        }
        ...
        private class XbimIPersistEntityCollectionEnumerator : IEnumerator<TType>
        {
            private readonly List<IEnumerator<TType>> _enumerators;
            public XbimIPersistEntityCollectionEnumerator(Dictionary<IModel, HashSet<TType>> collection)
            {
