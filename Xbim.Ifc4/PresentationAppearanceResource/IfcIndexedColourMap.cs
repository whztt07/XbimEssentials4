...
{
	[EntityName("IFCINDEXEDCOLOURMAP")]
	public  partial class @IfcIndexedColourMap : IfcPresentationItem, IInstantiableEntity
	{
		...
		internal IfcIndexedColourMap(IModel model) : base(model) 		{ 
			Model = model; 
			_colourIndex = new ItemSet<int>( model );
		}
		...
		private ItemSet<int> _colourIndex;
		#endregion
		...
		{ 
			get 
			{
				if(IsActivated) return _overrides;
		...
		{ 
			get 
			{
				if(IsActivated) return _colours;
		...
		public ItemSet<int> @ColourIndex 
		{ 
			get 
			{
				if(IsActivated) return _colourIndex;
