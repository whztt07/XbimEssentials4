...
namespace Xbim.Ifc4.StructuralElementsDomain
{
	...
	public  partial class @IfcTendon : IfcReinforcingElement, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _tensionForce;
		...
		{ 
			get 
			{
				if(IsActivated) return _preStress;
		...
		{ 
			get 
			{
				if(IsActivated) return _frictionCoefficient;
		...
		{ 
			get 
			{
				if(IsActivated) return _anchorageSlip;
		...
		{ 
			get 
			{
				if(IsActivated) return _minCurvatureRadius;
