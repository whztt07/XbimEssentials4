...
namespace Xbim.Ifc4.GeometryResource
{
	...
	public abstract partial class @IfcBSplineSurface : IfcBoundedSurface
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _uDegree;
		...
		{ 
			get 
			{
				if(IsActivated) return _vDegree;
		...
		{ 
			get 
			{
				if(IsActivated) return _controlPointsList;
		...
		{ 
			get 
			{
				if(IsActivated) return _surfaceForm;
		...
		{ 
			get 
			{
				if(IsActivated) return _uClosed;
		...
		{ 
			get 
			{
				if(IsActivated) return _vClosed;
