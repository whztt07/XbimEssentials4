...
namespace Xbim.Ifc4.ApprovalResource
{
	...
	public  partial class @IfcApproval : INotifyPropertyChanged, IfcResourceObjectSelect, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _timeOfApproval;
		...
		{ 
			get 
			{
				if(IsActivated) return _qualifier;
		...
		{ 
			get 
			{
				if(IsActivated) return _requestingApproval;
		...
		{ 
			get 
			{
				if(IsActivated) return _givingApproval;
