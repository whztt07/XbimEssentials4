...
{
	[EntityName("IFCLIGHTSOURCEGONIOMETRIC")]
	public  partial class @IfcLightSourceGoniometric : IfcLightSource, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _colourAppearance;
		...
		{ 
			get 
			{
				if(IsActivated) return _colourTemperature;
		...
		{ 
			get 
			{
				if(IsActivated) return _luminousFlux;
		...
		{ 
			get 
			{
				if(IsActivated) return _lightEmissionSource;
		...
		{ 
			get 
			{
				if(IsActivated) return _lightDistributionDataSource;
