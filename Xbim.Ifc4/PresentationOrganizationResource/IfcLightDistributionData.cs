...
{
	[EntityName("IFCLIGHTDISTRIBUTIONDATA")]
	public  partial class @IfcLightDistributionData : INotifyPropertyChanged, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _mainPlaneAngle;
		...
		{ 
			get 
			{
				if(IsActivated) return _secondaryPlaneAngle;
		...
		{ 
			get 
			{
				if(IsActivated) return _luminousIntensity;
