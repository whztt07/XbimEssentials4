...
{
	[EntityName("IFCCURRENCYRELATIONSHIP")]
	public  partial class @IfcCurrencyRelationship : INotifyPropertyChanged, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _relatingMonetaryUnit;
		...
		{ 
			get 
			{
				if(IsActivated) return _relatedMonetaryUnit;
		...
		{ 
			get 
			{
				if(IsActivated) return _exchangeRate;
		...
		{ 
			get 
			{
				if(IsActivated) return _rateDateTime;
		...
		{ 
			get 
			{
				if(IsActivated) return _rateSource;
