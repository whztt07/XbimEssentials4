...
{
	[EntityName("IFCRELINTERACTIONREQUIREMENTS")]
	public  partial class @IfcRelInteractionRequirements : IfcRelConnects, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _dailyInteraction;
		...
		{ 
			get 
			{
				if(IsActivated) return _importanceRating;
		...
		{ 
			get 
			{
				if(IsActivated) return _locationOfInteraction;
		...
		{ 
			get 
			{
				if(IsActivated) return _relatedSpaceProgram;
		...
		{ 
			get 
			{
				if(IsActivated) return _relatingSpaceProgram;
