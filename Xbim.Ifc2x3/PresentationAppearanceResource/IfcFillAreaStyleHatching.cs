...
{
	[EntityName("IFCFILLAREASTYLEHATCHING")]
	public  partial class @IfcFillAreaStyleHatching : IfcGeometricRepresentationItem, IfcFillStyleSelect, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _hatchLineAppearance;
		...
		{ 
			get 
			{
				if(IsActivated) return _startOfNextHatchLine;
		...
		{ 
			get 
			{
				if(IsActivated) return _pointOfReferenceHatchLine;
		...
		{ 
			get 
			{
				if(IsActivated) return _patternStart;
		...
		{ 
			get 
			{
				if(IsActivated) return _hatchLineAngle;
