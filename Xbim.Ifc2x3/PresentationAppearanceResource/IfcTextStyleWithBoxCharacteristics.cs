...
{
	[EntityName("IFCTEXTSTYLEWITHBOXCHARACTERISTICS")]
	public  partial class @IfcTextStyleWithBoxCharacteristics : INotifyPropertyChanged, IfcTextStyleSelect, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _boxHeight;
		...
		{ 
			get 
			{
				if(IsActivated) return _boxWidth;
		...
		{ 
			get 
			{
				if(IsActivated) return _boxSlantAngle;
		...
		{ 
			get 
			{
				if(IsActivated) return _boxRotateAngle;
		...
		{ 
			get 
			{
				if(IsActivated) return _characterSpacing;
