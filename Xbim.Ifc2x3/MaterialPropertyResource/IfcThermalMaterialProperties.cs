...
{
	[EntityName("IFCTHERMALMATERIALPROPERTIES")]
	public  partial class @IfcThermalMaterialProperties : IfcMaterialProperties, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _specificHeatCapacity;
		...
		{ 
			get 
			{
				if(IsActivated) return _boilingPoint;
		...
		{ 
			get 
			{
				if(IsActivated) return _freezingPoint;
		...
		{ 
			get 
			{
				if(IsActivated) return _thermalConductivity;
