...
{
	[EntityName("IFCMECHANICALCONCRETEMATERIALPROPERTIES")]
	public  partial class @IfcMechanicalConcreteMaterialProperties : IfcMechanicalMaterialProperties, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _compressiveStrength;
		...
		{ 
			get 
			{
				if(IsActivated) return _maxAggregateSize;
		...
		{ 
			get 
			{
				if(IsActivated) return _admixturesDescription;
		...
		{ 
			get 
			{
				if(IsActivated) return _workability;
		...
		{ 
			get 
			{
				if(IsActivated) return _protectivePoreRatio;
		...
		{ 
			get 
			{
				if(IsActivated) return _waterImpermeability;
