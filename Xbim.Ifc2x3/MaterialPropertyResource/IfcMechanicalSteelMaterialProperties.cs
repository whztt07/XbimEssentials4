...
{
	[EntityName("IFCMECHANICALSTEELMATERIALPROPERTIES")]
	public  partial class @IfcMechanicalSteelMaterialProperties : IfcMechanicalMaterialProperties, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _yieldStress;
		...
		{ 
			get 
			{
				if(IsActivated) return _ultimateStress;
		...
		{ 
			get 
			{
				if(IsActivated) return _ultimateStrain;
		...
		{ 
			get 
			{
				if(IsActivated) return _hardeningModule;
		...
		{ 
			get 
			{
				if(IsActivated) return _proportionalStress;
		...
		{ 
			get 
			{
				if(IsActivated) return _plasticStrain;
		...
		{ 
			get 
			{
				if(IsActivated) return _relaxations;
