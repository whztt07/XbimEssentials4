...
{
	[EntityName("IFCHYGROSCOPICMATERIALPROPERTIES")]
	public  partial class @IfcHygroscopicMaterialProperties : IfcMaterialProperties, IInstantiableEntity
	{
		...
		{ 
			get 
			{
				if(IsActivated) return _upperVaporResistanceFactor;
		...
		{ 
			get 
			{
				if(IsActivated) return _lowerVaporResistanceFactor;
		...
		{ 
			get 
			{
				if(IsActivated) return _isothermalMoistureCapacity;
		...
		{ 
			get 
			{
				if(IsActivated) return _vaporPermeability;
		...
		{ 
			get 
			{
				if(IsActivated) return _moistureDiffusivity;
